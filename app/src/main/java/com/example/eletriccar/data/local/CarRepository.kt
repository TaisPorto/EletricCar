...
class CarRepository(private val context: Context) {
    ...
    fun findCarById(id: Int): Carro {
        ...
        // LISTAGEM DAS COLUNAS AS SEREM EXIBIDAS NO RESULTADO DA QUERY
        val columns = arrayOf(
            BaseColumns._ID,
            COLUMN_NAME_CAR_ID,
            COLUMN_NAME_PRECO,
        ...
        with(cursor) {
            while (moveToNext()) {
                itemId = getLong(getColumnIndexOrThrow(COLUMN_NAME_CAR_ID)).toInt()
    ...
    // Incluindo dados na aba favoritos
    ...
    fun getAll(): List <Carro> {
        ...
        with(cursor) {
            while (moveToNext()) {
                 val itemId = getLong(getColumnIndexOrThrow(COLUMN_NAME_CAR_ID)).toInt()
                Log.d("ID ->", itemId.toString())
